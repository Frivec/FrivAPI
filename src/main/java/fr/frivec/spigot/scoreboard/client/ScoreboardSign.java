package fr.frivec.spigot.scoreboard.client;
...
public class ScoreboardSign {
	 ...
	 * 
	 * @author zyuiop
	 * Updated for 1.15.2 by Frivec
	 * 
	 */
	
	public static final Map<Player, ScoreboardSign> scoreboards = new HashMap<>();
	 ...
	 */
	public ScoreboardSign(Player player, ScoreboardPattern pattern) {
		this.player = player;
	...
	public void create() {
		...
		player.sendPacket(createObjectivePacket(0, this.pattern.getObjectiveName()));
		...
		for(Entry<Integer, String> entries: this.pattern.getLines().entrySet())
			...
			setLine(entries.getKey(), entries.getValue());
	...
	private PacketPlayOutScoreboardObjective createObjectivePacket(int mode, String displayName) {
		...
		if (mode == 0 || mode == 2) {
			setField(packet, "b", ChatSerializer.a("{\"text\":\"" + displayName + "\"}"));
	...
	public void applyPattern() {
		...
		for(Entry<Integer, String> entries: this.pattern.getLines().entrySet())
			...
			this.setLine(entries.getKey(), entries.getValue());
	...
	public static ScoreboardSign getCurrentScoreboard(Player player) {
